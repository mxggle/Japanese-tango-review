    ...
    <div className="bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-md p-4 shadow-md">
      <div className="max-w-7xl mx-auto flex items-center gap-4">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-lg shadow-xl border dark:border-slate-700 overflow-hidden z-10">
              <div className="p-2 space-y-1">
                <button
                    onClick={() => { scrollToLastWatched(); setIsMenuOpen(false); }}
                ...
                <button
                    onClick={() => { toggleAll(true); setIsMenuOpen(false); }}
                ...
                <button
                    onClick={() => { toggleAll(false); setIsMenuOpen(false); }}
                    className="w-full text-left px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 bg-transparent rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                >
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 p-2">
                <label className="px-2 pt-1 pb-2 block text-xs font-semibold text-slate-500 dark:text-slate-400">Filter by Type</label>
                <div className="grid grid-cols-2 gap-2 px-1">
                  {(['all', 'kanji', 'katakana', 'hiragana'] as const).map((filter) => {
                    ...
                    return (
                      <button
                        key={filter}
                        onClick={() => { setWordTypeFilter(filter); setIsMenuOpen(false); }}
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 p-2">
                 <div className="flex items-center justify-between gap-2 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 rounded-md">
                  <label htmlFor="blur-toggle" className="cursor-pointer flex-grow">Blur Furigana</label>
                  <button
                    role="switch"
